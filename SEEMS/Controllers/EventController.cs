...
public class EventController : ControllerBase
{
	...
	[HttpGet("is-updatable/{id}")]
	[CheckUserStatus]
	public async Task<ActionResult> IsUpdatable(int id)
	{
		var user = await GetCurrentUser(Request);
		var myEvent = _context.Events.FirstOrDefault(e => e.Id == id);
		if(myEvent != null)
		{
			var isUpdatable = myEvent.StartDate.Subtract(DateTime.Now).TotalHours >= 2;
			return Ok(
				new Response(
					ResponseStatusEnum.Success,
					new
					{
						CanTakeAttendance = isUpdatable
					}
				)
			);
		}
