...
public class UserMetaRepository : RepositoryBase<UserMeta>, IUserMetaRepository
{
	...
	public async Task<UserMeta> GetRolesAsync(string email, bool trackChanges) =>
		await FindByCondition(m => m.User.Email.Equals(email) && m.MetaKey.Equals("role"), trackChanges)
	...
	{
		var meta = _context.UserMetas.FirstOrDefault(m => m.UserId == userId && m.MetaKey.Equals(CONSECUTIVE_ABSENCES_KEY));
	...
	{
		var result = _context.UserMetas.FirstOrDefault(m => m.UserId == userId && m.MetaValue.Equals(CONSECUTIVE_ABSENCES_KEY)).MetaValue;
