...
namespace SEEMS.Services
{
	public class EventsServices
	{
		public static EventValidationInfo? GetValidatedEventInfo(EventDTO eventDTO)
		{
			...
			if(eventDTO.RegistrationDeadline != null && (eventDTO.StartDate.Subtract((DateTime) eventDTO.RegistrationDeadline).TotalHours < EventValidationInfo.MinHoursRegistrationFromStart || ((DateTime) eventDTO.RegistrationDeadline).Subtract(DateTime.Now).TotalHours < EventValidationInfo.MinHoursRegistrationFromNow))
			{
				failedCheck = true;
