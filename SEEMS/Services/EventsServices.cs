...
namespace SEEMS.Services
{
	public class EventsServices
	{
		public static EventValidationInfo? GetValidatedEventInfo(EventDTO eventDTO)
		{
			...
			if(eventDTO.RegistrationDeadline != null && eventDTO.StartDate.Subtract((DateTime) eventDTO.RegistrationDeadline).TotalHours < EventValidationInfo.MinHoursRegistrationDeadline)
			{
				failedCheck = true;
				validationInfo.RegistrationDeadline = $"Registration deadline must before start date at least {EventValidationInfo.MinHoursRegistrationDeadline} hours";
			}
